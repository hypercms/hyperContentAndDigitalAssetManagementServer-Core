...
function rdbms_settemplate ($object, $template)
{
  ...
  {    
    ...
    // update object
    $sql = 'UPDATE object SET template="'.$template.'" WHERE objectpath="'.$object.'"'; 
...
function rdbms_renameobject ($object_old, $object_new)
{
  ...
  if ($object_old != "" && $object_new != "" && (substr_count ($object_old, "%page%/") > 0 || substr_count ($object_old, "%comp%/") > 0) && (substr_count ($object_new, "%page%/") > 0 || substr_count ($object_new, "%comp%/") > 0))
  {
    ...
    // for object
    else $sql .= 'WHERE objectpath="'.$object_old.'"';;
    ...
    if ($done)
    {
      while ($row = $db->rdbms_getresultrow ('select'))
      {
        if (!empty ($row['object_id']))
        {
          ...
          // update object 
          if ($filetype != "") $sql = 'UPDATE object SET level='.getobjectpathlevel($object).', objectpath="'.$object.'", objectpathname="'.getobjectpathname($object).'", filetype="'.$filetype.'" WHERE object_id='.$object_id.'';
          else $sql = 'UPDATE object SET level='.getobjectpathlevel($object).', objectpath="'.$object.'", objectpathname="'.getobjectpathname($object).'" WHERE object_id='.$object_id.'';
    ...
    if (!empty ($expression_filename) && trim ($expression_filename) != "" && $expression_filename != "*Null*")
    {
      ...
      foreach ($temp_array as $temp_operator => $temp2_array)
      {
        foreach ($temp2_array as $temp_expression)
        {
          ...
          // decode special characters for the search in objectpath
          $temp_expression = specialchr_decode ($temp_expression);
          ...
          // if MATCH AGAINST query
          else
          {
            // replace underline since it is not present in the full-text search index
            $temp_expression = str_replace ("_", " ", $temp_expression);
          }
...
function rdbms_getobject_id ($object, $recreate=false)
{
  ...
  if (is_string ($object) && $object != "")
  {
    ...
    { 
      ...
      $sql = 'SELECT object_id, deleteuser FROM object WHERE objectpath="'.$object.'"';
    }
    // object hash
    else
    {
...
function rdbms_getobject_hash ($object="", $container_id="", $recreate=false)
{
  ...
  if ($object != "" || intval ($container_id) > 0)
  {
    ...
    if (substr_count ($object, "%page%/") > 0 || substr_count ($object, "%comp%/") > 0)
    {
      ...
      $sql = 'SELECT hash, deleteuser FROM object WHERE objectpath="'.$object.'" LIMIT 1';
    }
    // if object id
...
function rdbms_createrecipient ($object, $from_user, $to_user, $email)
{
  ...
  if ($object != "" && $from_user != "" && $to_user != "" && $email != "")
  {
    ...
    else $sql = 'SELECT object_id FROM object WHERE objectpath="'.$object.'"';
...
function rdbms_getrecipients ($object)
{
  ...
  if ($object != "")
  {
    ...
    // get recipients
    $sql = 'SELECT rec.recipient_id, rec.object_id, rec.date, rec.from_user, rec.to_user, rec.email FROM recipient AS rec INNER JOIN object AS obj ON obj.object_id=rec.object_id WHERE obj.objectpath="'.$object.'"';   
...
function rdbms_getnotification ($event="", $object="", $user="")
{
  ...
  if (is_array ($mgmt_config))
  {
    ...
    if ($object != "")
    {
      ...
      $sql = 'SELECT DISTINCT object_id, id FROM object WHERE objectpath="'.$object.'"';
...
//        return file size of objects [boolean] (optional), limit returned results [integer] (optional), cache timeout in seconds using 0 for no caching and -1 for permanent cache with no timeout or "auto" for automatic cache setting [integer] (optional)
...
function rdbms_getmediastat ($date_from="", $date_to="", $activity="", $container_id="", $objectpath="", $user="", $return_filesize=true, $maxhits=0, $cache_timeout="auto")
{
  ...
  // load and use permanent cached results with no timeout
  elseif (intval ($cache_timeout) < 0 && is_file ($mgmt_config['abs_path_view'].$filename))
  {
  ...
  // cache results permantently if no cached data has been loaded
  elseif (intval ($cache_timeout) < 0 && empty ($cached))
  {
    $json = json_encode ($dailystat);
    savefile ($mgmt_config['abs_path_view'], $filename, $json);
  }
...
// ------------------------------------------ rdbms_setobjectpathname ----------------------------------------------
// function: rdbms_setobjectpathname()
// input: %
// output: true / false

// description: 
// Update the objectpathname for support of version 10.0.8
...
function rdbms_setobjectpathname ()
{
  ...
  // select objects
  $sql = 'SELECT object_id, objectpath FROM object WHERE objectpathname=""';
  $errcode = "50802";
  $result = $db->rdbms_query ($sql, $errcode, $mgmt_config['today'], 'select');
