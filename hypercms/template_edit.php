...
if (checkglobalpermission ($site, 'template') && checkglobalpermission ($site, 'tpledit') && $save == "yes" && checktoken ($token, $user))
{
  ...
  {    
    ...
    {
      $add_onload = " hcms_openBrWindowItem('template_view.php?site=".url_encode($site)."&cat=".$cat."&template=".url_encode($template)."','preview','scrollbars=yes,resizable=yes', '800', '600');";
    }
    else $add_onload = "";
  }
...
function openHelp()
{
  hcms_openBrWindowItem('template_help.php?site=<?php echo $site; ?>', 'help', 'resizable=yes,scrollbars=yes', '750', '680');
}

function openmetaInfo()
{  
  hcms_openBrWindowItem('template_edit_metainfo.php?site=<?php echo $site; ?>', 'constraint', 'scrollbars=no,resizable=no', '450', '150');
}

function openLanguageInfo()
{  
  hcms_openBrWindowItem('template_edit_language.php?site=<?php echo $site; ?>', 'language', 'scrollbars=no,resizable=no', '450', '150');
}

function openConstraints()
{  
  hcms_openBrWindowItem('template_edit_constraints.php?site=<?php echo $site; ?>', 'constraint', 'scrollbars=no,resizable=no', '450', '250');
}

function openmediaType()
{  
  hcms_openBrWindowItem('template_edit_mediatype.php?site=<?php echo $site; ?>', 'constraint', 'scrollbars=no,resizable=no', '350', '150');
}
