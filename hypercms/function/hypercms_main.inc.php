...
function createaccesslink ($site, $location="", $object="", $cat="", $object_id="", $login="", $type="al", $lifetime=0, $formats="")
{
  ...
  if (!empty ($mgmt_config['api_checkpermission']) && valid_publicationname ($site) && valid_locationname ($location))
  {
...
// ---------------------------------------- editgroup --------------------------------------------
// function: editgroup()
// input: publication name [string], group name [string], page folder access array [array], component folder access array [array], permissions with permission name as key and value of 1 or 0 [array], user name [string]
// output: result array
...
function editgroup ($site, $groupname, $pageaccess, $compaccess, $permission, $user)
{
  global $eventsystem, $mgmt_config, $hcms_lang, $lang;
...
function createfolder ($site, $location, $folder, $user)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($folder) && !strpos ($folder, ".recycle") && accessgeneral ($site, $location, "") && strlen ($folder) <= $mgmt_config['max_digits_filename'] && valid_objectname ($user) && !is_tempfile ($folder))
  {
    ...
    // eventsystem
    if (!empty ($eventsystem['oncreatefolder_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function deletefolder ($site, $location, $folder, $user)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($folder) && accessgeneral ($site, $location, $cat) && $user != "")
  {
    ...
    // folder exists
    if (!empty ($folder) && is_dir ($location.$folder) && is_file ($location.$folder."/.folder"))
    {
    ...
    // eventsystem
    if (!empty ($eventsystem['ondeletefolder_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function contentcount ($user)
{
  ...
  if (empty ($filedata))
  {
    $temp = rdbms_externalquery ("SELECT MAX(id) AS contentcount FROM container");
          ...
          { 
            ...
            if ($test != false)
            {
              ...
              else
              {
                ...
                // eventsystem
                if (!empty ($eventsystem['oncreateobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function manipulateobject ($site, $location, $page, $pagenew, $user, $action, $clipboard_array=array())
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && accessgeneral ($site, $location, $cat) && valid_objectname ($user) && $action != "")
  {
    ...
    // eventsystem for paste
    if ($action == "page_paste" && $eventsystem['onpasteobject_post'] == 1 && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function cutobject ($site, $location, $page, $user, $clipboard_add=false, $clipboard_session=true)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($page) && valid_objectname ($user))
  {
    ...
    // eventsystem
    if (!empty ($eventsystem['oncutobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function copyobject ($site, $location, $page, $user, $clipboard_add=false, $clipboard_session=true)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($page) && valid_objectname ($user))
  {
    ...
    if (substr_count ($location, $mgmt_config['abs_path_rep']) == 0 || substr_count ($location, $mgmt_config['abs_path_comp'].$site."/") > 0)
    {
      ...
      // eventsystem
      if (!empty ($eventsystem['oncopyobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function copyconnectedobject ($site, $location, $page, $user, $clipboard_add=false, $clipboard_session=true)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($page) && valid_objectname ($user))
  {
    ...
    if (substr_count ($location, $mgmt_config['abs_path_rep']) == 0 || substr_count ($location, $mgmt_config['abs_path_comp'].$site."/") > 0)
    {
      ...
      // eventsystem
      if (!empty ($eventsystem['oncopyconnectedobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
function lockobject ($site, $location, $page, $user)
{
  ...
  if (valid_publicationname ($site) && valid_locationname ($location) && valid_objectname ($page) && valid_objectname ($user))
  {
    ...
    if ($pagedata != "")
    {
      ...
      // eventsystem
      if (!empty ($eventsystem['onlockobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
if (parent.frames['mainFrame']) parent.frames['mainFrame'].location.reload();";
      ...
      // eventsystem
      if (!empty ($eventsystem['onunlockobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
        ...
        if (is_array ($object_array) && sizeof ($object_array) > 0)
        {
          ...
          foreach ($object_array as $object)
          {
            ...
            if ($show == "" && $result_save == true && $release >= 3 && $application != "media"  && ($viewstore != "" || $application == "generator"))
            {
              ...
              // eventsystem
              if (!empty ($eventsystem['onpublishobject_post']) && empty ($eventsystem['hide']) && $error_switch == "no") 
...
// date [YYYY-MM-DD hh:mm]|name of script file [string]|type ["error", "warning" or "information"]|unique error code in script file [integer]|error message [string]
