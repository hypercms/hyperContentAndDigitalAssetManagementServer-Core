...
echo showtopbar ($hcms_lang['check-for-duplicates'][$lang], $lang, $button_close, "objFrame");
?>
...
if (!empty ($object_info['md5_hash']))
{
  ...
  $object_array = rdbms_getduplicatefiles ($object_info['md5_hash']);

  if (is_array ($object_array) && sizeof ($object_array) > 0)
  {    
    foreach ($object_array as $result)
    {
      $result['location_esc'] = $result['location'];
      $result['location'] = deconvertpath ($result['location_esc'], "file"); 
      ...
      if (valid_locationname ($result['location']) && valid_objectname ($result['object']) && is_file ($result['location'].$result['object']) && $file_info != false)
      {
        ...
        // location
        $location_obj_short = getlocationname ($result['publication'], $result['location_esc'], $result['category'], "path");  
        ...
        if ($result['category'] == "comp")
        {
          ...
          else
          {
            echo "
            <tr class=\"".$rowcolor."\">
              <td style=\"white-space:nowrap;\"><a href=\"javascript:void(0);\" ".(!$current_object ? "onclick=\"hcms_openWindow('".cleandomain ($mgmt_config['url_path_cms'])."page_preview.php?site=".url_encode($result['publication'])."&ctrlreload=yes&cat=".url_encode($result['category'])."&location=".url_encode($result['location_esc'])."&page=".url_encode($result['object'])."', 'preview', 'location=no,menubar=no,toolbar=no,titlebar=no,scrollbars=yes,resizable=yes,status=no', ".windowwidth("object").", ".windowheight("object").");\"" : "")."><img src=\"".getthemelocation()."img/".$file_info['icon']."\" class=\"hcmsIconList\" />&nbsp; ".$file_info['name']."</a></td>
              <td style=\"white-space:nowrap;\">".$location_obj_short."</td>
      ...
      {
        $errcode = "20101";
        $error[] = $mgmt_config['today']."|page_info_container.php|error|".$errcode."|The connected object does not exist '".$result['location_esc'].$result['object']."'";    
      }  
    }
  }
  else
  ...
  openBrWindow('popup_log.php?description=<p class=\"hcmsHeadline\">".getescapedtext ($hcms_lang['functional-error-occured'][$lang])."</p>".getescapedtext ($hcms_lang['link-management-database-is-corrupt-or-you-do-not-have-read-permissions'][$lang])."', 'alert', 'location=no,menubar=no,toolbar=no,titlebar=no,scrollbars=yes,status=no,width=600,height=220','600','220');
    ...
    $errcode = "20102";
    $error[] = $mgmt_config['today']."|page_info_duplicates.php|error|".$errcode."|Error in rdbms_getduplicatefiles for publication '".$site."'";        
  }
}
