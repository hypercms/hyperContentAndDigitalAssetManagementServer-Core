...
function updateCodeSegment()
{
  ...
  if (zoom == true) zoom_action = "onclick=\"var n=this.firstChild.childNodes; if(this.style.width=='90%') { this.style.cssText='transition:width .5s; " + width + height + "'; this.firstChild.style.backgroundSize='cover'; if(n[0]) n[0].style.display='block'; if(n[1]) n[1].style.display='block'; } else { this.style.cssText='position:fixed; z-index:9999; width:90%; height:90%; top:50%; left:50%; transform:translate(-50%, -50%); transition:width 1s;'; this.firstChild.style.backgroundSize='contain'; if(n[0]) n[0].style.display='none'; if(n[1]) n[1].style.display='none'; }\"";
  else zoom_action = "";
