...
if ($pagestore != false)
{
  ...
  if ($contentfile != "" && $template != "")
  {
    ...
    if (!empty ($contentfile))
    {
      ...
      echo "<tr><td valign=top>".$hcms_lang['owner'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=top>".$owner[0]."</td></tr>\n";
      echo "<tr><td valign=top>".$hcms_lang['modified'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=top>".$last_updated[0]."</td></tr>\n";
      echo "<tr><td valign=top>".$hcms_lang['published'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=top>".$last_published[0]."</td></tr>\n"; 
    }
    ...
    {
      echo "<tr><td valign=\"top\">".$hcms_lang['will-be-removed'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=\"top\">".substr ($queue[0]['date'], 0, -3)."</td></tr>\n";
    }
    
    // container
    if (!empty ($contentfile))
    {
      echo "<tr><td valign=top>".$hcms_lang['container-id'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=top>".$container_id."</td></tr>\n";    
    }
    ...
    if ($cat == "comp")
    {
      ...
      if ($filesize > 0) echo "<tr><td valign=\"top\">".$hcms_lang['file-size'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=\"top\">".$filesize."</td></tr>\n";
      if ($filecount > 1) echo "<tr><td valign=\"top\">".$hcms_lang['number-of-files'][$lang].": </td><td class=\"hcmsHeadlineTiny\" valign=\"top\">".$filecount."</td></tr>\n";
    }
...
if (!empty ($template) && $template != false)
{
  ...
  if ($wf_exists == true && file_exists ($mgmt_config['abs_path_data']."workflow/".$site."/".$contentfile))
  {
    ...
    echo "<p class=\"hcmsHeadline\">".$hcms_lang['workflow-status'][$lang]."</p>\n";
    echo "<table border=0 cellspacing=2 cellpadding=3 width=\"90%\">\n";
    echo "<tr>
    <td class=\"hcmsHeadline\">".$hcms_lang['member-type'][$lang]."</td>
    <td class=\"hcmsHeadline\">".$hcms_lang['member'][$lang]."</td>
    <td class=\"hcmsHeadline\">".$hcms_lang['status'][$lang]."</td>
    <td class=\"hcmsHeadline\">".$hcms_lang['date'][$lang]."</td>
  </tr>\n"; 
    ...
    for ($stage=$stage_start; $stage<=$stage_max; $stage++)
    {
      ...
      {
        echo "<tr class=\"hcmsRowHead2\"><td colspan=\"4\">".$hcms_lang['members-on-workflow-stage'][$lang]." ".$stage."</td></tr>\n"; 
        ...
        foreach ($item_array[$stage] as $item)
        {
          ...
          {
            $type = $hcms_lang['user'][$lang];
            $member_array = getcontent ($item, "<user>"); // unique
          }
          elseif ($type_array[0] == "usergroup")
          {
            $type = $hcms_lang['user-group'][$lang];
            $member_array = getcontent ($item, "<group>"); // unique
          }
          elseif ($type_array[0] == "script") 
          {
            $type = $hcms_lang['robot-script'][$lang];
            $member_array[0] = "-";
          }                      
          
          $passed_array = getcontent ($item, "<passed>"); // unique
          
          if ($passed_array[0] == 1) $passed = $hcms_lang['accepted'][$lang];
          else $passed = $hcms_lang['pendingrejected'][$lang];
