...
$reportname = getrequest_esc ("reportname", "objectname");
...
{
  if (valid_objectname ($reportname))
  {
    $result = createreport ($reportname);
...
elseif ($action == "item_delete")
{
  ...
  {
    $result = deletereport ($reportfile);
...
{  
  ...
  if (form.elements['reportname'].value == "")
  {
    alert (hcms_entity_decode("<?php echo getescapedtext ($hcms_lang['a-name-is-required'][$lang]); ?>"));
    form.elements['reportname'].focus();
    return false;
  }
  
  if (!checkForm_chars (form.elements['reportname'].value, "-_"))
  {
    form.elements['reportname'].focus();
    return false;
  }
...
<div class="hcmsLocationBar">
  <table border=0 cellspacing=0 cellpadding=1>
    <tr>
      <td><b><?php echo getescapedtext ($hcms_lang['report-management'][$lang]); ?></b></td>
    </tr>
    <tr>
      <td>&nbsp;</td>
    </tr>  
...
<div class="hcmsToolbar">
  ...
  <div class="hcmsToolbarBlock">
    <?php
    if (file_exists ($mgmt_config['abs_path_cms']."help/reportguide_".$hcms_lang_shortcut[$lang].".pdf"))
    {echo "<img  onClick=\"hcms_openWindow('../help/reportguide_".$hcms_lang_shortcut[$lang].".pdf', 'help', 'scrollbars=no,resizable=yes', 800, 600);\" name=\"pic_obj_help\" src=\"".getthemelocation()."img/button_help.png\" class=\"hcmsButton hcmsButtonSizeSquare\" alt=\"".getescapedtext ($hcms_lang['help'][$lang])."\" title=\"".getescapedtext ($hcms_lang['help'][$lang])."\" />\n";}
    elseif (file_exists ($mgmt_config['abs_path_cms']."help/reportguide_en.pdf"))
    {echo "<img  onClick=\"hcms_openWindow('../help/reportguide_en.pdf', 'help', 'scrollbars=no,resizable=yes', 800, 600);\" name=\"pic_obj_help\" src=\"".getthemelocation()."img/button_help.png\" class=\"hcmsButton hcmsButtonSizeSquare\" alt=\"".getescapedtext ($hcms_lang['help'][$lang])."\" title=\"".getescapedtext ($hcms_lang['help'][$lang])."\" />\n";}
    ?>
  </div>
</div>
  ...
  <table width="100%" height="60" border="0" cellspacing="2" cellpadding="0">
    <tr>
      <td valign="middle" nowrap="nowrap">
        ...
        <?php echo getescapedtext ($hcms_lang['report'][$lang]." ".$hcms_lang['name'][$lang]); ?>: 
        <input type="text" name="reportname" maxlength="100" style="width:220px;" />
  ...
  <table width="100%" height="60" border="0" cellspacing="2" cellpadding="0">
    <tr>
      <td valign="middle" nowrap="nowrap">
        ...
        <?php echo getescapedtext ($hcms_lang['report'][$lang]); ?>:
        <select name="reportfile" onChange="hcms_jumpMenu('parent.frames[\'mainFrame\']',this,0)">
          <option value="../empty.php">--- <?php echo getescapedtext ($hcms_lang['select'][$lang]); ?> ---</option>
          <?php
          $item_files = getdirectoryfiles ($mgmt_config['abs_path_data']."report/", ".report.dat");
          $item_option_edit = array();

          if (is_array ($item_files))
          {
             foreach ($item_files as $value)
             {
               if (strpos ($value, ".report.dat") > 0) $item_name = substr ($value, 0, strpos ($value, ".report.dat"));

               echo "<option value=\"report_form.php?save=no&preview=yes&reportfile=".url_encode($value)."\">".$item_name."</option>\n";

               $item_option_edit[] = "<option value=\"report_form.php?save=no&preview=no&reportfile=".url_encode($value)."\">".$item_name."</option>\n";
             }
          }
          ?>
        </select>
