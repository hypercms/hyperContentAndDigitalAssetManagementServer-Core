          ...
          if ($ownergroup != false && $setlocalpermission['root'] == 1 && valid_locationname ($location) && valid_objectname ($object) && is_file ($location.$object))
          {
            ...
            if (!empty ($container_id))
            {
              ...
              if ($mediafile != false)
              {
                ...
                if (empty ($file_size) && !empty ($objectlistcols_reduced['filesize']))
                {
                  // location of file
              ...
              {
                // get file time
                if (!empty ($objectlistcols_reduced['date'])) $file_modified = date ("Y-m-d H:i", @filemtime ($location.$object));

                // get file size
                if (!empty ($objectlistcols_reduced['filesize']))
                {
...
<link rel="stylesheet" href="<?php echo cleandomain ($mgmt_config['url_path_cms']); ?>theme/print.css" media="print" />
