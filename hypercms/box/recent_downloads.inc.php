...
if (is_array ($object_array) && sizeof ($object_array) > 0)
{
  ...
  <div id=\"recent_downloads\" class=\"hcmsHomeBox\" style=\"text-align:left; margin:10px; width:".$width."; height:400px;\">
    <div class=\"hcmsHeadline\" style=\"margin:6px 2px;\"><img src=\"".getthemelocation("night")."img/button_file_download.png\" class=\"hcmsIconList\" /> ".getescapedtext ($hcms_lang['my-recent-downloads'][$lang])."</div>";
    ...
    if (getobject ($objectpath) != ".folder")
    {
      ...
      if (valid_publicationname ($item_site) && $item_cat != "")
      {
        ...
        if ($ownergroup != false && $setlocalpermission['root'] == 1 && valid_locationname ($item_location) && valid_objectname ($item_object))
        {
          ...
          echo "
          <div ".$openObject." style=\"display:block; cursor:pointer;\" title=\"".$item_locationname.$item_fileinfo['name']."\"><img src=\"".getthemelocation()."img/".$item_fileinfo['icon']."\" class=\"hcmsIconList\" />&nbsp;<a href=\"javascript:void(0);\">".showshorttext($item_fileinfo['name'], 30)."</a>&nbsp;</div>";
          $i++;
        }
      }
    }
