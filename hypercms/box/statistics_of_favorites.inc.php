...
if (isset ($siteaccess) && is_array ($siteaccess))
{
  ...
  $object_array = getfavorites ($user, "path");
  ...
  if (!empty ($object_array) && is_array ($object_array) && sizeof ($object_array) > 0)
  {
    foreach ($object_array as $item_objectinfo)
    {
      ...
      if ($item_objectpath != "")
      {
        ...
        echo "
        <div id=\"stats_".$item_object."\" onclick=\"hcms_openWindow('frameset_content.php?site=".url_encode($item_site)."&ctrlreload=yes&cat=".url_encode($item_cat)."&location=".url_encode($item_location)."&page=".url_encode($item_object)."', '".$item_object."', 'location=no,menubar=no,toolbar=no,titlebar=no,location=no,status=yes,scrollbars=no,resizable=yes,titlebar=no', ".windowwidth ("object").", ".windowheight ("object").");\" class=\"hcmsHomeBox\" style=\"text-align:left; cursor:pointer; margin:10px; width:".$width."; height:400px; overflow:auto; ".($is_iphone ? "-webkit-overflow-scrolling:touch;" : "")."\">
          <div class=\"hcmsHeadline\" style=\"margin:6px 2px; white-space:nowrap;\"><img src=\"".getthemelocation("night")."img/favorites.png\" class=\"hcmsIconList\" /> ".(empty ($is_mobile) ? getescapedtext ($hcms_lang['access-statistics-for'][$lang]) : "")." ".showshorttext($item_fileinfo['name'], 40, false)."</div>
