...
// input: view of object, hypertag to create view of inlcuded objects, view parameter, application  
      ...
      if ($includedata != false)
      {
        ...
        if (@substr_count (strtolower ($includedata), "hypercms:fileinclude") > 0)
        {
          ...
          if ($hypertag_array != false)
          {
            ...
            {
              $viewstore = viewinclusions ($site, $viewstore, $hypertag, $view, $application);
            }
          }
        }
...
//        force category to use different location path for %url_location% or %abs_location% [page,comp] (optional), execute_code [treu/false] (optional
//
  ...
  {  
    ...
    { 
      ...
      {  
        ...
        $viewstore = viewinclusions ($site, $viewstore, $hypertag, $inclview, $application);                    
      }
    }
                    ...
                    if (is_array ($bufferarray)) 
                    {
                      ...
                      foreach ($bufferarray as $data) 
                      {
                        ...
                        if (!empty ($tmpid[0]) && !empty ($tmpcontent[0])) 
                        {
                          ...
                          if ($tmpuser[0] == $user || $adminpermission || $globalpermission[$site]['user'] == 1)
                          {
                    ...
                    elseif ($searchtag == "artmedia")
                    {
                      ...
                      elseif ($buildview == "formedit" || ($buildview == "formmeta" && $infotype[$id] == "meta") || $buildview == "formlock")
                      {
                        if ($buildview == "formedit" || $buildview == "formmeta")
                        {
                          ...
                          <img onClick=\"setSaveType('form_so', '".$mgmt_config['url_path_cms']."media_edit_frameset.php?view=".url_encode($buildview)."&savetype=form_so&site=".url_encode($site)."&cat=".url_encode($cat)."&location=".url_encode($location_esc)."&page=".url_encode($page)."&db_connect=".url_encode($db_connect)."&id=".url_encode($id)."&label=".url_encode($label[$id])."&tagname=media&mediacat=comp&mediafile=".url_encode($mediafilebot[$id])."&mediaobject_curr=".url_encode($mediaobjectbot[$id])."&mediaobject=' + getValue('".$hypertagname_file[$id]."[".$id."]','') + '&mediaalttext=' + getValue('".$hypertagname_text[$id]."[".$id."]','*Null*') + '&mediaalign=' + getSelectedOption('".$hypertagname_align[$id]."[".$id."]','*Null*') + '&mediawidth=' + getValue('".$hypertagname_width[$id]."[".$id."]','*Null*') + '&mediaheight=' + getValue('".$hypertagname_height[$id]."[".$id."]','*Null*') + '&mediatype=".$mediatype[$id]."&contenttype=".$contenttype."');\" class=\"hcmsButtonTiny hcmsButtonSizeSquare\" src=\"".getthemelocation()."img/button_media.gif\" align=\"absmiddle\" alt=\"".$text6[$lang]."\" title=\"".$text6[$lang]."\" />\n";
                        }
                        else $taglink = "";
...
<body class=\"hcmsWorkplaceGeneric\">\n";
  ...
  function hcms_changeitem (tagname, id, condition, pos, type)
  {
    ...
    else if (type == 'send') 
    {
      ...
      document.forms['hcms_result'].attributes['action'].value = 'component_edit_frameset.php';
      changes = true;
    }
