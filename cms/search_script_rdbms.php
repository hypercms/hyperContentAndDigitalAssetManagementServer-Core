    ...
    { 
      ...
      if (substr_count ($objectpath, "/") > 0)
      {
        ...
        if (valid_publicationname ($item_site) && $item_cat != "")
        {
          ...
          {           
            // page
            if ($item_info['type'] == "Page") $item_type = $text4[$lang];
            // component
            ...
            {              
              ...
              if (@is_file ($mediadir.$item_site."/".$media_info['filename'].".thumb.jpg") && @filesize ($mediadir.$item_site."/".$media_info['filename'].".thumb.jpg") > 400)
              {
                ...
                $thumbnail = "<div id=\"".strtolower(substr($ratio, 0, 1)).$items_row."\" class=\"hcmsThumbnail".$ratio.$temp_explorerview."\"><img src=\"".$mgmt_config['url_path_cms']."explorer_wrapper.php?site=".url_encode($item_site)."&media=".url_encode($item_site."/".$media_info['filename'].".thumb.jpg")."&token=".hcms_crypt($item_site."/".$media_info['filename'].".thumb.jpg")."\" ".$class_image." /></div>";
              }
            ...
            if ($mediafile != false && is_array ($_SESSION['hcms_linking']) && $setlocalpermission['root'] == 1 && $setlocalpermission['download'] == 1)
            {
              ...
              $linking_buttons = "<div style=\"width:".$width."; margin-left:auto; margin-right:auto; padding:0; text-align:center;\"><a href=\"".$mgmt_config['url_path_cms']."explorer_wrapper.php?name=".$object_name."&media=".$item_site."/".$mediafile."&token=".hcms_crypt($item_site."/".$mediafile)."\" target=\"_blank\"><button class=\"hcmsButtonDownload\">".$text23[$lang]."</button></a><a href=\"".$mgmt_config['url_path_cms']."explorer_download.php?name=".$object_name."&media=".$item_site."/".$mediafile."&token=".hcms_crypt($item_site."/".$mediafile)."\"><button class=\"hcmsButtonDownload\">".$text24[$lang]."</button></a></div>";
            }
            else $linking_buttons = "";            
  
            $galleryview .= "<td id=t".$items_row." width=\"".$cell_width."\" align=\"center\" valign=\"bottom\">
                              <div ".$selectclick." ".$openObject." ".$hcms_setObjectcontext." title=\"".$metadata."\" style=\"cursor:pointer;\">".
                                $dlink_start."
                                 ".$thumbnail."</br />
