...
if (valid_objectname ($media) && ((hcms_crypt ($media) == $token && ($user != "" || is_thumbnail ($media, false) || !$mgmt_config[$site]['dam'])) || $media_approved))
{
  ...
  if ($media_root != "")
  {
    ...
    if ($media != "")
    {
      ...
      // save and close hyperCMS session and start new LiveView session to secure the hyperCMS session
      session_write_close ();
      session_name ('LiveView');
      // read cookie if available to reuse existing LiveView session
      if (isset ($_COOKIE['liveview'])) session_id ($_COOKIE['liveview']);
      else session_id (sha1 (mt_rand()));
      // set cookie to save session id for LiveView, valid for 4 hours
      setcookie ('liveview', session_id(), time()+60*60*4);
